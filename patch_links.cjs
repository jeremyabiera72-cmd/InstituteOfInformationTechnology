const fs = require('fs');
let code = fs.readFileSync('src/pages/SharedLinks.tsx', 'utf8');
...
const deleteRegex = /const handleDelete = async \(id: number\) => {[\s\S]*?alert\('Error deleting link'\);\s*}\s*};/;
...
const buttonRegex = /onClick=\{\(\) => handleDelete\(link.id\)\}/;
code = code.replace(buttonRegex, "onClick={() => setConfirmAction({ type: 'delete', id: link.id })}");
...
fs.writeFileSync('src/pages/SharedLinks.tsx', code);
console.log("Patched SharedLinks.tsx");
